...
// Pin declaration
#define BTN_MUSIC 16 // 25
#define BTN_LICHT 26
#define BTN_WASSER 25 // 27
#define BTN_STOP 17   // 14
#define PTI_SPEED 39  // 15
...
void handlePoti()
{
  ...
  int speed = 0;
  if (ptiSpeed > 1100)
    speed = 64; // Fast foreward
  else if (ptiSpeed > 800)
    speed = 32; // normal forweard
  else if (ptiSpeed > 600)
    speed = 16; // slow forefard (might not work on low battery)
  else if (ptiSpeed > 400)
    speed = 0; // stop
  else if (ptiSpeed > 200)
    speed = -32; // slow backward
  else
    speed = -64; // fast foreward
